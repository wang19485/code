...
private:
    ...
    BlockHeader* getbuddy (BlockHeader * addr) {
        BlockHeader*buddy=(BlockHeader*)((((char*)addr-(char*)total)^addr->block_size)+(char*)total);
        return buddy;
    }
	// given a block address, this function returns the address of its buddy 
	
    bool arebuddies (BlockHeader* block1, BlockHeader* block2){
        if ((block1 < block2 && (char*)block1 + block1->block_size == (char*)block2) || (block1 > block2 && (char*)block1 - block1->block_size == (char*)block2)) {
            return 1;
        }else{
            return 0;
        }
    ...
    BlockHeader* split (BlockHeader* block){
        char *ad = (char*)block + block->block_size/2;
        return (BlockHeader*)ad;
    }
