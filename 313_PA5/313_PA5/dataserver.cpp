...
void process_file_request (NetworkRequestChannel* rc, char* request){
    ...
    //cout << nbytes << "  " << f->length << endl;
	assert (nbytes == f->length);
	rc->cwrite (buffer, nbytes);
	fclose (fp);
}

void process_data_request (NetworkRequestChannel* rc, char* request){
	datamsg* d = (datamsg* ) request;
	double data = get_data_from_memory (d->person, d->seconds, d->ecgno);
	rc->cwrite((char *) &data, sizeof (double));
}

void process_unknown_request(NetworkRequestChannel *rc){
	char a = 0;
	rc->cwrite (&a, sizeof (a));
}


int process_request(NetworkRequestChannel *rc, char* _request)
{
...
void handle_process_loop(NetworkRequestChannel *channel)
{
	for (;;){
...
int main(int argc, char *argv[])
{
	...
	NetworkRequestChannel* control_channel = new NetworkRequestChannel (port, handle_process_loop);
