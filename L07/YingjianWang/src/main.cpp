...
static void render()
{
    ...
    MV->translate(0, -shape_ground->maxy, 0);
    MV->scale(10, 1, 10);
    MV->translate(0.25, 0, -0.25);
    ...
    MV->translate(L.light1);
    MV->scale(0.7, 0.7, 0.7);
    ...
    int count = 0;
    float sc = sin(glfwGetTime())*1/8+1;
    for (int j = 0 ; j < 5; j++) {
        for (int i = 0; i < 4; i++) {
            P->pushMatrix();
            camera->applyProjectionMatrix(P);
            ...
            camera->applyViewMatrix(MV);
            if (colorv[count][3] == 1) {
                MV->translate(i+1, -shape->miny*sc, -1-j);
                MV->scale(sc, sc, sc);
            }else{
                MV->translate(i+1, -shape2->miny*sc, -1-j);
                MV->scale(sc, sc, sc);
            }
